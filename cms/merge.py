...
if __name__ == "__main__":
    ...
    print "Running NanoAOD" 
    flist = glob("result/201804*lam/step3.root")
    for f in flist:
        d = os.path.dirname(f)
        # os.system("cd %s && V0Sim step3.root" % d)
        # os.system("hadd -f9 %s %s %s" % (d+"/nv.root", d+"/nanoAOD.root", d+"/v0sim.root"))
        os.system("cd %s && cp ../../hadAOD17.py . && cmsRun hadAOD17.py" % d)
    print "Merging HADOAD"
    flist = glob("result/201804*lam/hadAOD.root")
    os.system("hadd -f9 cmssw_ttswbw_hadAOD.root "+" ".join(flist))
